...
###############Model 3-Regression with Linear Trend and Seasonality with AR(8) Model
...
## USE Arima() FUNCTION TO CREATE AR(8) MODEL FOR TRAINING RESIDUALS.
## CREATE TWO-LEVEL MODEL WITH LINEAR TREND AND SEASONALITY MODEL 
## AND AR(8) RESIDUALS.
## PLOT DATA AND IDENTIFY ACCURACY MEASURES.

# Use Arima() function to fit AR(8) model for training residuals. The Arima model of 
# order = c(8,0,0) gives an AR(8) model.
# Use summary() to identify parameters of AR(1) model. 
res.ar1 <- Arima(train.lin.trend.season$residuals, order = c(8,0,0))
summary(res.ar1)
...
# regression + AR(8) for residuals for validation period.

# Create data table with validation data, regression forecast
# for validation period, AR(8) residuals for validation, and 
...
names(valid.df) <- c("Sales", "Reg.Forecast", 
                     "AR(8)Forecast", "Combined.Forecast")
valid.df
...
plot(valid.two.level.pred, 
     ...
     main = "Two-Level Forecast: Regression with Trend
             and Seasonlity + AR(8) for Residuals", 
     lty = 2, lwd = 2, col = "blue")
axis(1, at = seq(2010, 2024, 1), labels = format(seq(2010, 2024, 1)) )
lines(train.lin.trend.season.pred$fitted, col = "blue", lwd = 2)
lines(train.ts, col = "black", lty = 1)
...
#######################
###############Model 5- Regression with Quadratic Trend and Seasonality with AR(8) Model
...
# Use Arima() function to fit AR(8) model for training residuals. The Arima model of 
# order = c(8,0,0) gives an AR(1) model.
# Use summary() to identify parameters of AR(8) model. 
res.ar1.quad <- Arima(train.quad.trend.season$residuals, order = c(8,0,0))
...
plot(valid2.two.level.pred, 
     ...
     main = "Two-Level Forecast: Regression with Quadratic Trend
             and Seasonality + AR(8) for Residuals", 
     lty = 2, lwd = 2, col = "blue")
axis(1, at = seq(2010, 2024, 1), labels = format(seq(2010, 2024, 1)) )
lines(train.quad.trend.season.pred$fitted, col = "blue", lwd = 2)
lines(train.ts, col = "black", lty = 1)
...
# Use accuracy() function to identify common accuracy measures for validation period forecast:
# (1) two-level model (linear trend and seasonal model + AR(8) model for residuals),
# (2) linear trend and seasonality model only.
...
Acf(HW.ZZZ.pred$residuals, lag.max = 12, 
    main = "Autocorrelations of Residuals for Automated Holt-Winter's Model with entire dataset")
...
Acf(lin.trend.season.pred$residuals, lag.max = 12, 
    main = "Autocorrelations of Residuals of Regression model based on Linear Trend and Seasonality Model with Entire Data Set")

############### Model 3 ------------------- Regression with linear Trend and Seasonality with with AR(8) Model
...
# Use Arima() function to fit AR(8) model for regression residuals.
# The ARIMA model order of order = c(8,0,0) gives an AR(1) model.
# Use forecast() function to make prediction of residuals into the future 12 months.
residual.lin.ar1 <- Arima(lin.trend.season$residuals, order = c(8,0,0))

residual.lin.ar1.pred <- forecast(residual.lin.ar1, h = 12, level = 0)
residual.lin.ar1.pred
...
# AR(8) model for residuals for 12 future periods, and combined 
...
names(table1.df) <- c("Reg.Forecast", "AR(8)Forecast","Combined.Forecast")
table1.df
...
plot(sales.ts, 
     xlab = "Time", ylab = "Sales (in million $)", ylim = c(25000, 90000), 
     bty = "l", xlim = c(2010, 2023.25), xaxt = "n",
     main = "Two-Level Forecast: Regression with Linear Trend and Seasonlity + AR(8)
...
# Use Arima() function to fit AR(8) model for regression residuals.
# The ARIMA model order of order = c(1,0,0) gives an AR(8) model.
# Use forecast() function to make prediction of residuals into the future 12 months.
residual.quad.ar1 <- Arima(quad.trend.season$residuals, order = c(8,0,0))

residual.quad.ar1.pred <- forecast(residual.quad.ar1, h = 12, level = 0)
residual.quad.ar1.pred
...
# seasonal model and AR(8) model for residuals.
quad.season.ar1.pred <- quad.trend.season.pred$mean + residual.quad.ar1.pred$mean
...
names(table2.df) <- c("Reg.Forecast", "AR(8)Forecast","Combined.Forecast")
table2.df
...
plot(sales.ts, 
     xlab = "Time", ylab = "Sales (in million $)", ylim = c(25000, 90000), 
     bty = "l", xlim = c(2010, 2023.25), xaxt = "n",
     main = "Two-Level Forecast: Regression with Quadratic Trend and Seasonality + AR(8)
     for Residuals", lwd = 2, col = "black")
axis(1, at = seq(2010, 2024, 1), labels = format(seq(2010, 2024, 1)) )
